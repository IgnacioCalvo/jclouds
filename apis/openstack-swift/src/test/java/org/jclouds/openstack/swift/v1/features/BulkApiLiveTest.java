...
public class BulkApiLiveTest extends BaseSwiftApiLiveTest<SwiftApi> {
   ...
   public void testExtractArchive() throws Exception {
      for (String regionId : regions) {
         ...
         assertEquals(api.getContainerApi(regionId).get(containerName).getObjectCount(), new Long(OBJECT_COUNT));
   ...
   public void testBulkDelete() throws Exception {
      for (String regionId : regions) {
         ...
         assertTrue(deleteResponse.getErrors().isEmpty());
         assertEquals(api.getContainerApi(regionId).get(containerName).getObjectCount(), new Long(0));
      }
   }
